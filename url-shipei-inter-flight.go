...
func GetUrl(u string, s map[string]string) {
	...
	for _, v := range v.Loc {
		...
		if len(sHtml) == 5 {
			...
			fp.WriteString("<data>\n<display>\n<html5_url><![CDATA[")
			fp.WriteString("http://m.ctrip.com/html5/Flight/international/" + path[2] + "-all/")
			fp.WriteString("]]>\n</html5_url>\n</display>\n</data>\n</url>\n")
			/* 适配只出一条
			fp.WriteString("\t\t<url>\n\t\t\t<loc>")
			fp.WriteString("http://m.ctrip.com/html5/Flight/international/" + path[2] + "-direct/")
			fp.WriteString("</loc>\n\t\t<mobile:mobile/>\n\t\t</url>\n")

			fp.WriteString("\t\t<url>\n\t\t\t<loc>")
			fp.WriteString("http://m.ctrip.com/html5/Flight/international/single-" + sHtml[1] + "-" + sHtml[2] + "-" + sHtml[3] + "-" + sHtml[4] + "-all/")
			fp.WriteString("</loc>\n\t\t<mobile:mobile/>\n\t\t</url>\n")

			fp.WriteString("\t\t<url>\n\t\t\t<loc>")
			fp.WriteString("http://m.ctrip.com/html5/Flight/international/single-" + sHtml[1] + "-" + sHtml[2] + "-" + sHtml[3] + "-" + sHtml[4] + "-direct/")
			fp.WriteString("</loc>\n\t\t<mobile:mobile/>\n\t\t</url>\n")
			*/
		}
	}
...
func main() {
	...
	GetUrl("http://flights.ctrip.com/inter_searchresult.xml", header)
